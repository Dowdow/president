...
// transformFamilySymbol returns the plain Unicode suit character, without the
// emoji variation selector transformFamily uses for the DOM UI — SDF text
// renderers (e.g. drei's <Text> in the 3D scene) need the plain glyph to
// reliably resolve against a regular font.
export function transformFamilySymbol(family: CardFamilyType): string {
  switch (family) {
    case CardFamily.Spade:
      return "♠";
    case CardFamily.Heart:
