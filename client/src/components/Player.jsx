...
export default function Player({ player, totalPlayers }) {
  const cardsLeft = Array.isArray(player.cards)
    ? player.cards.length
    : player.cards;
  return (
    <div
      className={`flex items-center mb-5 ${player.playing ? "ml-5" : ""}`}
      style={{ order: player.order }}
    >
      <div
      ...
      >
        ...
        <span>
          {[...Array(cardsLeft)].map((c, i) => (
            <span
              key={i}
              className="inline-block w-2 h-3 m-0.5 bg-white border-2 border-black rounded shadow"
            />
          ))}
        </span>
      </div>
